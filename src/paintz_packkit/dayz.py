...
def _owner_class(pack: dict, dayz: dict, local_pack_class: str) -> str:
...
def emit_dayz(
    ...
    *,
    ...
    namespace_role = dayz.get("namespace_role", "owner")
    if namespace_role not in {"owner", "satellite"}:
        raise ValueError(f"Unsupported PaintZ namespace role: {namespace_role!r}")

    local_pack_class = _local_pack_class(pack)
    owner_class = _owner_class(pack, dayz, local_pack_class)
    ...
    role_label = "standalone owner" if namespace_role == "owner" else "satellite content"
    lines = [
